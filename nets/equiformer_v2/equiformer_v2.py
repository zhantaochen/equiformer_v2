...
class EquiformerV2(BaseModel):
    ...
    def __init__(
        self,
        num_atoms=None,     # not used, place holder for BaseModel
        bond_feat_dim=None, # not used, place holder for BaseModel
        num_targets=None,   # not used, place holder for BaseModel
        use_pbc=True,
        regress_forces=True,
        otf_graph=True,
        output_aggr_mode="node",
        output_block_channels=1,
        max_neighbors=500,
        max_radius=5.0,
        ...
        self.output_block_channels = output_block_channels
        ...
        self.output_aggr_mode = output_aggr_mode
        assert self.output_aggr_mode in ['mean', 'node']
    ...
    def forward(self, data):
        ...
        # Output estimation
        ###############################################################
        node_output = self.output_block(x) 
        node_output = node_output.embedding.narrow(dim=1, start=0, length=1)
        if self.output_aggr_mode == 'mean':
            output = torch.zeros((len(data.natoms), self.output_block_channels), device=node_output.device, dtype=node_output.dtype)
            output.index_add_(dim=0, index=data.batch, source=node_output.view(-1, self.output_block_channels))
            output = output / _AVG_NUM_NODES
